...
type threefish512 struct {
	expandedTweak      [EXPANDED_TWEAK_SIZE]uint64
	expandedKey        [EXPANDED_KEY_SIZE_512]uint64
	tmpData1, tmpData2 [CIPHER_QWORDS_512]uint64
}

// Get an initialized Threefish512 structure
func newThreefish512(key []byte, tweak []uint64) (*threefish512, error) {
	c := new(threefish512)

	// Initialize tweak
	setTweak(tweak, c.expandedTweak[:])

	c.expandedKey[EXPANDED_KEY_SIZE_512-1] = KEY_SCHEDULE_CONST
	...
	if key != nil {
		...
		setKey(tmpkey[:], c.expandedKey[:])
	}
	return c, nil
}

// Get an initialized Threefish512 structure
func newThreefish512_64(key, tweak []uint64) (*threefish512, error) {
	c := new(threefish512)
