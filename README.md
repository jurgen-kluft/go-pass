...
And we can show passwords too:
    ...
    jay@mac ~ $ pass email/jay_at_gmail.com
    sup3rh4x3rizmynam3
    

Or copy them to the clipboard:

    jay@mac ~ $ pass -c email/jay_at_gmail.com
    Copied email/jay_at_gmail.com to clipboard. Will clear in 45 seconds.
...
To begin, there is a single command to initialize the password store:
    ...
    jay@mac ~ $ pass init "jay_at_gmail Password Storage Key"
    mkdir: created directory ‘/home/jay_at_gmail/.password-store’
    Password store initialized for jay_at_gmail Password Storage Key.
    

Here, `jay_at_gmail Password Storage Key` is the ID of my GPG key. You can use your standard GPG key or use an alternative one especially for the password store as shown above. Multiple GPG keys can be specified, for using pass in a team setting, and different folders can have different GPG keys, by using `-p`.

We can additionally initialize the password store as a git repository:

    jay@mac ~ $ pass git init
    Initialized empty Git repository in /home/jay_at_gmail/.password-store/.git/
    jay@mac ~ $ pass git remote add origin kexec.com:pass-store
    

If a git repository is initialized, `gpw` creates a git commit each time the password store is manipulated.

There is a more [detailed initialization example](https://git.jay_at_gmail.com/password-store/about/#EXTENDED%20GIT%20EXAMPLE) in the [man page](https://git.jay_at_gmail.com/password-store/about/).
...
* [Version 1.7.4](https://git.jay_at_gmail.com/password-store/snapshot/password-store-1.7.4.tar.xz)
* [Latest Git](https://git.jay_at_gmail.com/password-store/snapshot/password-store-master.tar.xz)

The tarball contains a generic makefile, for which a simple `sudo make install` should do the trick.

### Git Repository

You may [browse the git repository](https://git.jay_at_gmail.com/password-store/) or clone the repo:

    $ git clone https://git.jay_at_gmail.com/password-store

All releases are tagged, and the tags are signed with [0xA5DE03AE](http://www.jay_at_gmail.com/keys/AB9942E6D4A4CFC3412620A749FC7012A5DE03AE.asc).
...
* [passmenu](https://git.jay_at_gmail.com/password-store/tree/contrib/dmenu): an **extremely useful and awesome** dmenu script
...
* [pass.applescript](https://git.jay_at_gmail.com/password-store/tree/contrib/pass.applescript): OS X integration
* [pass-git-helper](https://github.com/languitar/pass-git-helper): git credential integration
* [password-store.el](https://git.jay_at_gmail.com/password-store/tree/contrib/emacs): an emacs package
...
* [1password2pass.rb](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/1password2pass.rb): imports 1Password txt or 1pif data
* [keepassx2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/keepassx2pass.py): imports KeepassX XML data
* [keepass2csv2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/keepass2csv2pass.py): imports Keepass2 CSV data
* [keepass2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/keepass2pass.py): imports Keepass2 XML data
* [fpm2pass.pl](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/fpm2pass.pl): imports Figaro's Password Manager XML data
* [lastpass2pass.rb](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/lastpass2pass.rb): imports Lastpass CSV data
* [kedpm2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/kedpm2pass.py): imports Ked Password Manager data
* [revelation2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/revelation2pass.py): imports Revelation Password Manager data
* [gorilla2pass.rb](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/gorilla2pass.rb): imports Password Gorilla data
* [pwsafe2pass.sh](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/pwsafe2pass.sh): imports PWSafe data
* [kwallet2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/kwallet2pass.py): imports KWallet data
* [roboform2pass.rb](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/roboform2pass.rb): imports Roboform data
* [password-exporter2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/password-exporter2pass.py): imports password-exporter data
* [pwsafe2pass.py](https://git.jay_at_gmail.com/password-store/tree/contrib/importers/pwsafe2pass.py): imports pwsafe data
...
`gpw` was written by [Jason A. Donenfeld](mailto:Jason@jay_at_gmail.com) of [jay_at_gmail.com](http://jay_at_gmail.com) and is licensed under the [GPLv2](http://www.gnu.org/licenses/gpl-2.0.html)+.

### Contributing

This is a very active project with a [healthy dose of contributors](https://git.jay_at_gmail.com/password-store/stats/?period=y&ofs=-1). The best way to contribute to the password store is to [join the mailing list](http://lists.jay_at_gmail.com/listinfo.cgi/password-store-jay_at_gmail.com) and send git formatted patches. You may also join the discussion in `#pass` on Libera.Chat.